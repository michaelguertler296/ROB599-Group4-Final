...
from pose_utils import normalize, look_at, poses_avg, recenter_poses, to_float, generate_spiral_cam_to_world, generate_spherical_cam_to_world, flatten, generate_one_Pose
from torch.utils.data import Dataset, DataLoader


def get_dataset(dataset_name, base_dir, split, factor, device, poseRadius, poseTheta):
    d = dataset_dict[dataset_name](base_dir, split, factor=factor, device=device, poseRadius=poseRadius, poseTheta=poseTheta)
    return d


def get_dataloader(dataset_name, base_dir, split, poseRadius, poseTheta, factor=4, batch_size=None, shuffle=True, device=torch.device("cpu")):
    d = get_dataset(dataset_name, base_dir, split, factor, device, poseRadius, poseTheta)
...
class NeRFDataset(Dataset):
    def __init__(self, base_dir, split, poseRadius, poseTheta, spherify=False, near=2, far=6, white_bkgd=False, factor=1, n_poses=120, radius=None, radii=None, h=None, w=None, device=torch.device("cpu")):
        super(Dataset, self).__init__()
        self.base_dir = base_dir
        self.split = split
        self.poseRadius = poseRadius
        self.poseTheta = poseTheta
    ...
    def generate_render_poses(self):
        ...
        if self.spherify:
            #self.generate_onePose(self.poseTheta, self.poseRadius); 
            self.generate_spherical_poses(self.n_poses)
        else:
            #self.generate_onePose(self.poseTheta, self.poseRadius);
            self.generate_spiral_poses(self.n_poses)

    def generate_onePose(self, poseTheta, poseRadius):
        self.poses = generate_one_Pose(self.poseTheta, self.poseRadius)
...
class Blender(NeRFDataset):
    """Blender Dataset."""
    def __init__(self, base_dir, split, poseRadius, poseTheta, factor=1, spherify=False, white_bkgd=True, near=2, far=6, radius=4, radii=1, h=800, w=800, device=torch.device("cpu")):
        super(Blender, self).__init__(base_dir, split, poseRadius, poseTheta, factor=factor, spherify=spherify, near=near, far=far, white_bkgd=white_bkgd, radius=radius, radii=radii, h=h, w=w, device=device)
